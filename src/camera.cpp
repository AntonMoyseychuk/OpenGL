...
void camera::rotate(float angle_radians, const glm::vec2& axis) noexcept {
    m_thi += axis.y * glm::degrees(angle_radians);
    m_theta += axis.x * glm::degrees(angle_radians);

    const auto forward_x = m_radius * std::sinf(glm::radians(m_theta)) * std::sinf(glm::radians(m_thi));
    const auto forward_y = m_radius * std::cosf(glm::radians(m_theta));
    const auto forward_z = m_radius * std::sinf(glm::radians(m_theta)) * std::cosf(glm::radians(m_thi));
...
float & camera::get_speed() noexcept {
    return m_speed;
}
...
float &camera::get_sensitivity() noexcept {
    return m_sensitivity;
}

float camera::get_sensitivity() const noexcept {
    return m_sensitivity;
}
...
glm::vec3 &camera::get_position() noexcept {
    return m_position;
}
